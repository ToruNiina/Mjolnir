...
template<typename traitsT>
GJFNPTLangevinIntegrator<traitsT>
read_GJFNPT_langevin_integrator(const toml::value& simulator)
{
    MJOLNIR_GET_DEFAULT_LOGGER();
    MJOLNIR_LOG_FUNCTION();
    using real_type = typename traitsT::real_type;
    ...
    check_keys_available(integrator, {"type"_s, "seed"_s, "remove"_s, "env"_s,
            "box_alpha", "box_mass", "volume_velocity", "parameters"_s, "alphas"_s});

    const auto box_alpha = toml::find<real_type>(integrator, "box_alpha");
    const auto box_mass  = toml::find<real_type>(integrator, "box_mass");
    const auto vol_velo  = toml::find<real_type>(integrator, "volume_velocity");
    ...
    return GJFNPTLangevinIntegrator<traitsT>(
            delta_t, box_alpha, box_mass, vol_velo,
            std::move(alpha), read_system_motion_remover<traitsT>(simulator));
}
...
template<typename traitsT>
struct read_integrator_impl<GJFNPTLangevinIntegrator<traitsT>>
{
    static GJFNPTLangevinIntegrator<traitsT> invoke(const toml::value& sim)
    {
        return read_GJFNPT_langevin_integrator<traitsT>(sim);
    }
};
...
extern template GJFNPTLangevinIntegrator<SimulatorTraits<double, CuboidalPeriodicBoundary>> read_GJFNPT_langevin_integrator(const toml::value& simulator);
extern template GJFNPTLangevinIntegrator<SimulatorTraits<float,  CuboidalPeriodicBoundary>> read_GJFNPT_langevin_integrator(const toml::value& simulator);

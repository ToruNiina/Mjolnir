#ifndef MJOLNIR_OMP_GJ_F_NPT_LANGEVIN_INTEGRATOR_HPP
#define MJOLNIR_OMP_GJ_F_NPT_LANGEVIN_INTEGRATOR_HPP
...
class GJFNPTLangevinIntegrator<OpenMPSimulatorTraits<realT, boundaryT>>
{
  public:
...
#pragma omp parallel for reduction(+:vir)
        for(std::size_t i=0; i<sys.size(); ++i)
        {
            vir += math::dot_product(sys.force(i), sys.position(i));
        }
        this->virial_ = vir;
